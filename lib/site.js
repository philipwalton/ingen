...
module.exports = _.assign(site, {
  ...
  build: function() {
    ...
    config.contentTypes.forEach(function(type) {
      var typePlural = inflector.pluralize(type)
      _this[typePlural].forEach(function(file) {
        _this.emit(type, file.filename)
      })
    })
    ...
    this.files.forEach(function(filename) {
      ...
      if (file.data) {
        ...
        fs.writeFileSync(destination, fs.readFileSync(filename))

        site.emit("afterCopy", file)

  ...
  _walkPosts: function() {
    ...
    postTypes.forEach(function(postType) {
      ...
      postTypeFiles.forEach(function(filename) {
        ...
        var page = new Page(File.getOrCreate(filename), postType)

        // extract taxonomy data from the page and add it to the site object
        _.each(taxonomies, function(taxonomyValues, taxonomyTypePlural) {
          if (page[taxonomyTypePlural]) {
            taxonomies[taxonomyTypePlural] = _.union(
              taxonomyValues,
              page[taxonomyTypePlural]
            )
          }
        })

...
function Site(config) {
  ...
  this.posts = new PostManager(config.taxonomyTypes)
...
_.assign(Site.prototype, {
  ...
  _walkPosts: function() {
    ...
    _.each(postTypes, function(postType) {
      ...
      _.each(postTypeFiles, function(filename) {
        ...
        if (file.data.draft && !config.includeDrafts) return

        // create post objects for each file within a post type directory
        var post = new Post(file, postType)
        ...
        // (unless the post is a draft and publishing drafts is turned off)
  ...
  _walkFiles: function() {
    ...
    files.push.apply(files, config.includeFiles)
  ...
  _loadPlugins: function() {
    ...
    // don't load sytem plugins if they've been excluded
    systemPlugins = _.without.apply(_, [systemPlugins].concat(config.excludePlugins))
    ...
    _.each(localPlugins, function(filename) {
      require(path.resolve(config.pluginDirectory, filename)).call(_this)
    })
  },
  ...
  _clearCaches: function() {
    delete this.posts // maybe this needs to be reset to a PostManager instance
    Page.reset()
    Taxonomy.reset()
  },

...
module.exports = _.assign(site, {
  ...
  _walkPosts: function() {
    ...
    // create page objects for each file with a post type
  ...
  // TODO: some of this is repeated in
  // `_setupTemplateData` try to abstract it later
  _sortPostsByTaxonomy: function() {
    var _taxonomies = this._taxonomies
      , _this = this
    this._eachPost(function(post) {
      _this.config.taxonomies.forEach(function(txn) {
        var txnPlural = inflector.pluralize(txn)
        if (post[txnPlural] && post[txnPlural].length) {
          post[txnPlural].forEach(function(value) {
            var type = (_taxonomies[txnPlural] = _taxonomies[txnPlural] || {})
            type[value] = type[value] || []
            type[value].push(post)
          })
        }
      })
    })
  ...
  _setupTemplateData: function() {
    ...
    // populate those newly created arrays with post and taxonomy data
    this._eachPost(function(post) {

      // add the posts
      data[post.typePlural] = data[post.typePlural] || []
      data[post.typePlural].push(post.toTemplateObject())

      // add the taxonomies
      taxonomies.forEach(function(txn) {
        var txnPlural = inflector.pluralize(txn)
        if (post[txnPlural] && post[txnPlural].length) {
          post[txnPlural].forEach(function(value) {
            var type = (data[txnPlural] = data[txnPlural] || {})
            type[value] = type[value] || []
            type[value].push(post.toTemplateObject())
          })
        }
      })
    })

...
Post.prototype.init = function(obj, type, config) {
  ...
  this.config = config;
  this.permalink = this.template.resolvePermalink({type: this.type});
  this.filetype = this.template.filename
    ? path.extname(this.template.filename)
    : '.html'
}


Post.prototype.getTemplateData = function() {
  return _.assign({}, this.template.data, {site: this.config}, {

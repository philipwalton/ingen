...
function Query(params, posts) {
  ...
  this.posts = posts || this.posts || Post.all()
}
...
Query.prototype.filter = function() {
  ...
  this.posts = _.filter(this.posts, function(post) {
    ...
    _.each(taxonomyParams, function(taxonomyValue, taxonomyType) {
      ...
      // calling taxonomy.toString will woth both on strings and on Taxonomy instances
      var postTaxonomyValues = _.map(post[pluralize(taxonomyType)], function(taxonomy) {

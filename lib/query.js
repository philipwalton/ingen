...
Query.prototype.filter = function() {
  ...
  this.posts = _.filter(this.posts, function(post) {
    ...
    _.each(taxonomyParams, function(taxonomyValue, taxonomyType) {
      ...
      var postTaxonomyValues = post.data[pluralize(taxonomyType)];
...
Query.prototype.sort = function() {
  ...
  var postsHaveDates = !!(this.posts[0] && this.posts[0].data.date);
  ...
  this.posts.sort(function(a,b) {
    ...
    if (sortBy == 'date') {
      return moment(a.data.date).isBefore(b.data.date) ? -1 : 1;
    }

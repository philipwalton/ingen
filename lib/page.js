...
Page.prototype.init = function(obj, config) {
  ...
  if (obj instanceof Post) {
    ...
    Page.super_.prototype.init.call(this, obj, config)
  }
}
...
Page.prototype.paginate = function(posts) {
  ...
  var unpagedQuery = new Query(_.omit(this.query, 'limit', 'page'), posts)
  ...
  while (++page <= totalPages) {
    ...
    pages.push(new Page(obj, this.config))
  }
...
Page.prototype.render = function() {
  ...
  this.content = (function fn(layout, content, data) {
    layout = Template.getOrCreateFromFile(
      config.layoutsDirectory + '/' + layout + '.html',
      config
    )
...
Page.prototype._templateData = function() {
  return {
    site: this.config,
    // TODO: consider putting `page` this at the root of this object
    // so templates don't have to say page.* to access its properties
    page: this,

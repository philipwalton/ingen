...
var processFile = _.memoize(function(filename) {
  ...
  if (!data.layout) {
    throw new Error('No layout specified in "' + filename + '"')
  }
...
function parseJSON(content, filefilename) {
  var json
  try {
    json = JSON.parse(content)
  } catch(e) {
    throw new Error('Unable to parse JSON data from "' + filefilename + '"')
  }
  return json
}
